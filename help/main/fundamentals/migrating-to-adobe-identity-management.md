...
### &#x200B;1. 確定您的系統管理員和IT團隊，並討論他們可能需要採取的行動 {#identify-your-system-administrators}

* 如果您不確定組織中的系統管理員是誰，請聯絡您的 Adobe 帳戶團隊，或聯絡 Adobe 支援`marketocares@marketo.com`。

* 確認您的 Adobe Marketo Engage 訂閱將遷移至哪個 Adobe Admin Console (或 Adobe 組織)。 您可能具備適用於 [Dynamic Chat](/help/main/dynamic-chat/dynamic-chat-overview.md){target="_blank"} 的 Adobe Admin Console，這是 Adobe Marketo Engage 中的原生交談自動化工具。 Adobe Marketo Engage 訂閱必須與 Dynamic Chat 部署在同一個組織中。
...
### &#x200B;2. 熟悉移轉至Adobe Identity的變更和影響 {#familiarize-yourself-with-the-changes}
...
### &#x200B;3. 宣佈內部團隊所需的移轉時間表與準備工作 {#announce-the-migration-timeline}
...
i. 您已收到一封電子郵件驗證要求電子郵件，內含連結（3天有效）。 如果您的連結已過期，您可以從 Adobe Marketo Engage 中重新寄送驗證電子郵件，請按一下「我的設定檔」圖示，然後導覽至&#x200B;**「我的帳戶」**>**「帳戶設定」**>**「重新寄送驗證」**。
